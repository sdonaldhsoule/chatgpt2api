...
def stream_image_response(image_outputs: Iterable[ImageOutput], prompt: str, model: str, mode: str) -> Iterator[dict[str, Any]]:
    response_id = f"resp_{uuid.uuid4().hex}"
    created = int(time.time())
    yield response_created(response_id, model, created)
    for output in image_outputs:
        ...
        if items:
            item = items[0]
            usage = build_image_usage(prompt, len(items))
            if any(isinstance(image, dict) and image.get("b64_json") for image in output.data):
                try:
...
def response_events(body: dict[str, Any]) -> Iterator[dict[str, Any]]:
    ...
    yield from stream_image_response(image_outputs, prompt, model, mode)
